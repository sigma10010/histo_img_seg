...
def crop_image_to_tiles(img_path, output_dir, tile_size=(512, 512)):
    ...
    for top in range(0, img_height, tile_height):
        for left in range(0, img_width, tile_width):
            ...
            if 'mask' in base_name.split('_'):
                tile_name = f"{base_name.split('_')[0]}_tile_{count}_mask.bmp"
                tile.save(os.path.join(output_dir, tile_name), format="bmp")
            else:
...
# 示例调用
batch_crop_folder(
    input_dir='../datasets/monuseg/original_training/tissue_Images/',         # 输入图像文件夹
    output_dir='../datasets/monuseg/original_training/patches/s250/',  # 输出 tile 保存目录
    tile_size=(250, 250)        # 切片尺寸
)
