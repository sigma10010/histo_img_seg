...
safs = UNet(n_classes=2, reduction_ratio=None, n_head = 2, att_mode = 'bam', is_scale_selective = True, is_shortcut = True, conv_type = 'basic')
...
# 计算 FLOPs 和参数量
print('model: sask')
flops, params = profile(sask, inputs=(input_tensor, ))
print(f"FLOPs: {flops / 1e9:.2f} GFLOPs")
print(f"Params: {params / 1e6:.2f} M")

print('model: saba')
flops, params = profile(saba, inputs=(input_tensor, ))
print(f"FLOPs: {flops / 1e9:.2f} GFLOPs")
print(f"Params: {params / 1e6:.2f} M")

print('model: TransUNet')
flops, params = profile(TransUNet, inputs=(input_tensor, ))
print(f"FLOPs: {flops / 1e9:.2f} GFLOPs")
print(f"Params: {params / 1e6:.2f} M")

print('model: swinunet')
flops, params = profile(swinunet, inputs=(input_tensor, ))
print(f"FLOPs: {flops / 1e9:.2f} GFLOPs")
print(f"Params: {params / 1e6:.2f} M")
