...
if __name__ == '__main__':
    ...
    if config.model_type =='U_Net':
        ...
        unet = UNet(img_ch=3, n_classes=config.n_classes, init_features=config.width, network_depth=config.depth, reduction_ratio=None, n_skip=config.n_skip, activation = torch.nn.Softmax(dim=1))
    elif config.model_type =='SKU_Net':
        config.conv_type = 'sk'
        unet = UNet_V1(img_ch=3, n_classes=config.n_classes, init_features=config.width, network_depth=config.depth, reduction_ratio=None, conv_type = config.conv_type, activation = torch.nn.Softmax(dim=1))
    elif config.model_type =='SK-SC-U_Net':
        config.conv_type = 'sk'
        unet = UNet_V1(img_ch=3, n_classes=config.n_classes, init_features=config.width, network_depth=config.depth, reduction_ratio=None, is_shortcut = True, conv_type = config.conv_type, activation = torch.nn.Softmax(dim=1))
    elif config.model_type =='SK-SE-U_Net':
        config.conv_type = 'sk'
        config.att_mode = 'se'
        unet = UNet_V1(img_ch=3, n_classes=config.n_classes, init_features=config.width, network_depth=config.depth, reduction_ratio=config.reduction_ratio, att_mode = config.att_mode, is_shortcut = True, conv_type = config.conv_type, activation = torch.nn.Softmax(dim=1))
    elif config.model_type =='SEU_Net':
        config.att_mode = 'se'
    ...
    elif config.model_type =='CBAMU_Net':
        config.att_mode = 'cbam'
        unet = UNet(img_ch=3, n_classes=config.n_classes, init_features=config.width, network_depth=config.depth, reduction_ratio=config.reduction_ratio, att_mode = config.att_mode, activation = torch.nn.Softmax(dim=1))
    elif config.model_type =='BAMU_Net':
        config.att_mode = 'bam'
        unet = UNet(img_ch=3, n_classes=config.n_classes, init_features=config.width, network_depth=config.depth, reduction_ratio=config.reduction_ratio, att_mode = config.att_mode, activation = torch.nn.Softmax(dim=1))
    ...
    elif config.model_type == 'SSU_Net':
        unet = UNet_V4(img_ch=3, n_classes=config.n_classes, n_head = config.n_head, is_scale_selective = True, is_shortcut = False, activation = torch.nn.Softmax(dim=1))
